...
object BlurUtils {
    ...
    /**
     * Fast box/stack blur implementation that doesn't depend on deprecated RenderScript.
     * Scales down bitmap, applies multiple blur passes, and overlays a subtle dark tint.
     */
    fun blur(src: Bitmap, radius: Int = 16, darkTintAlpha: Int = 140): Bitmap {
        val width = (src.width / 4).coerceAtLeast(1)
        val height = (src.height / 4).coerceAtLeast(1)

        val scaled = Bitmap.createScaledBitmap(src, width, height, true)
        val blurred = fastBlur(scaled, radius)

        // Overlay ambient dark tint to match deep cinematic canvas #0b0c10
        val canvas = Canvas(blurred)
        val paint = Paint().apply {
            color = Color.argb(darkTintAlpha.coerceIn(0, 255), 11, 12, 16)
        }
    ...
    private fun fastBlur(sentBitmap: Bitmap, radius: Int): Bitmap {
        ...
        for (curX in 0 until w) {
            ...
            for (curY in 0 until h) {
                ...
                if (curX == 0) {
                    vmin[curY] = Math.min(curY + r1, hm) * w
                }
                p = curX + vmin[curY]
